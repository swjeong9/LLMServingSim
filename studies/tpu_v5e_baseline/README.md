...
| 측정 path          | inf2_baseline                           | tpu_v5e_baseline                          |
| ------------------ | --------------------------------------- | ------------------------------------- |
| vendor first-party | NxDI (AWS Neuron)                       | **MaxText** (Google JAX)        |
| open production    | vLLM-Neuron                             | **vLLM-TPU** (Pallas attention) |
| simulator          | LLMServingSim (Inferentia2 perf bundle) | LLMServingSim (TPU-v5e-1 perf bundle) |

## Layout

```
studies/tpu_v5e_baseline/
├── data/
...
`profiler/perf/TPU-v5e-1/<model>/<variant>/tp{N}/` 가 있어야 함
(TPU notebook 측정 → v2 변환 결과. 이 commit 에 포함).

```
profiler/perf/TPU-v5e-1/meta-llama/Llama-3.2-1B-Instruct/bf16/
├── meta.yaml
└── tp1/
...
```bash
python studies/tpu_v5e_baseline/convert_workload.py --all
```
...
> # studies/tpu_v5e_baseline/workloads → symlink 이므로 자동 공유
> ```
...
cat /tmp/sim_matrix.txt | xargs -n3 -P${PARALLEL} bash -c '
  tp=$0; ds=$1; bs=$2
  rel_out=studies/tpu_v5e_baseline/results/sim/Llama-3.2-1B/tp${tp}/bs${bs}/${ds}.csv
  ...
  docker run --rm \
    ...
    bash -c "
      ...
      python -m serving \
        --cluster-config configs/cluster/tpu_v5e_llama1b_tp${tp}.json \
        --dataset studies/tpu_v5e_baseline/workloads/${ds}_bs${bs}.jsonl \
        --output ${rel_out} \
        --max-num-seqs ${bs} \
...
```bash
tail -f studies/tpu_v5e_baseline/results/sim/Llama-3.2-1B/tp*/bs*/*.log
...
# 로컬 (Mac) 에서
rsync -av --progress \
    ubuntu@cpu-instance:~/LLMServingSim/studies/tpu_v5e_baseline/results/sim/ \
    studies/tpu_v5e_baseline/results/sim/
```
...
```bash
python studies/tpu_v5e_baseline/compare.py --tps 1 --batch-sizes 1,2,4,8,16,32
