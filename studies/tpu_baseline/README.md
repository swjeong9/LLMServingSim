...
```bash
python studies/tpu_baseline/convert_workload.py --all
...
* `measure_tpu.py`  — LENS NxD-direct (continuous batching off,
...
| `measure_tpu.py`            | `neuronx_distributed_inference` | `aws_neuronx_venv_pytorch_2_9_nxd_inference`                                                           |
...
# results/lens_tpu/ pulls log + CSV together). pipefail so the
# python exit code (not tee's) decides done vs FAIL.
set -o pipefail
run_nxd() {
  local tp=$1 bs=$2 ds=$3 cd=$4
  local dir=studies/tpu_baseline/results/lens_tpu/Llama-3.2-1B-Instruct/tp${tp}/bs${bs}
  mkdir -p ${dir}
  echo "=== [tp${tp} bs${bs} ${ds}] start $(date +%T) ==="
  if python studies/tpu_baseline/measure_tpu.py \
...
run_vllm() {
  local tp=$1 bs=$2 ds=$3 cd=$4
  local dir=studies/tpu_baseline/results/lens_vllm/Llama-3.2-1B-Instruct/tp${tp}/bs${bs}
  mkdir -p ${dir}
  echo "=== [tp${tp} bs${bs} ${ds}] start $(date +%T) ==="
  if python studies/tpu_baseline/measure_vllm.py \
      --dataset ${ds} --batch-size ${bs} \
      --model ~/models/Llama-3.2-1B-Instruct \
      --tp-degree ${tp} --max-model-len 8192 \
      --compiled-dir ${cd} 2>&1 | tee ${dir}/${ds}.log
...
python studies/tpu_baseline/measure_tpu.py --dataset cnn --batch-size 1 \
...
> ```bash
> python studies/tpu_baseline/convert_workload.py --all
> # → studies/tpu_baseline/workloads/{ds}_bs{N}.jsonl 24개
> ```
>
...
cat /tmp/sim_matrix.txt | xargs -n3 -P${PARALLEL} bash -c '
  tp=$0; ds=$1; bs=$2
  rel_out=studies/tpu_baseline/results/sim/Llama-3.2-1B/tp${tp}/bs${bs}/${ds}.csv
  ...
  docker run --rm \
    ...
    bash -c "
      ...
      python -m serving \
        ...
        --dataset studies/tpu_baseline/workloads/${ds}_bs${bs}.jsonl \
        --output ${rel_out} \
        --max-num-seqs ${bs} \
...
```bash
tail -f studies/tpu_baseline/results/sim/Llama-3.2-1B/tp*/bs*/*.log
...
```bash
# 로컬 (Mac) 에서
scp -r ubuntu@cpu-instance:~/LLMServingSim/studies/tpu_baseline/results/sim/ \
       studies/tpu_baseline/results/sim/

# 또는 sync (이미 일부 있는 경우)
rsync -av --progress \
    ubuntu@cpu-instance:~/LLMServingSim/studies/tpu_baseline/results/sim/ \
    studies/tpu_baseline/results/sim/
```
...
aws s3 sync studies/tpu_baseline/results/ \
...
```bash
python studies/tpu_baseline/compare.py --tp 1 --batch-sizes 1,2,4,8,16,32
python studies/tpu_baseline/compare.py --tp 2 --batch-sizes 1,2,4,8,16,32
...
* **NKI attention_cte kernel bypass — `attn_kernel_enabled=False`**:
  ...
  `measure_tpu.py` / `measure_vllm.py` 에 `attn_kernel_enabled=False`
