...
결과 위치: `studies/tpu_baseline/results/lens_tpu/Llama-3.2-1B-Instruct/tp1/bs<B>/<dataset>.csv`

호스트 (Mac) 로 회수:
```bash
# 로컬에서
rsync -avL v6e-1:~/LLMServingSim/studies/tpu_baseline/results/lens_tpu/ \
    studies/tpu_baseline/results/lens_tpu/
```
...
```bash
python studies/tpu_baseline/compare.py --tps 1 --batch-sizes 1
```

출력 표 + figure (`studies/tpu_baseline/figures/`):
