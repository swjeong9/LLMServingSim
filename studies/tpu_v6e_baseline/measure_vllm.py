#!/usr/bin/env python3
"""vLLM-TPU measurement — vLLM framework path on TPU.

TPU equivalent of studies/inf2_baseline/measure_vllm.py. Same heterogeneous
batch eval loop and output schema; differences:

  * No `NEURON_COMPILED_ARTIFACTS` / `override_neuron_config` —
    vLLM's TPU backend manages JAX compilation cache internally.
  * No NKI-specific knobs (attn_kernel_enabled, etc.) — TPU path uses
    Pallas attention kernels.
  * `dtype="bfloat16"`, prefix cache + chunked prefill both off (parity
    with measure_nxd.py / measure_vllm.py on inf2 so the comparison is
    apples-to-apples — same per-batch, no in-batch scheduler eviction).

Input  : studies/tpu_v6e_baseline/data/datasets/<dataset>.csv
Output : studies/tpu_v6e_baseline/results/lens_vllm/<model>/tp<N>/bs<B>/<dataset>.csv

Usage
-----
    python studies/tpu_v6e_baseline/measure_vllm.py \\
        --dataset arxiv --batch-size 4 \\
        --model meta-llama/Llama-3.2-1B-Instruct \\
        --tp-degree 1 --max-model-len 8192

vLLM-TPU prerequisites on the host:
    pip install vllm   # with TPU extras: pip install "vllm[tpu]"
    # PJRT auto-discovers TPU. Set explicitly if needed:
...
def load_dataset_csv(path: Path, batch_size: int):
    """Same logic as inf2_baseline — top batch_size * NUM_BATCHES rows,
    re-grouped into NUM_BATCHES batches."""
...
def init_llm(model, tp_degree, batch_size, max_model_len):
    ...
    comparison with the inf2 measurements."""
...
def main():
    ...
    with out_json.open("w") as f:
        json.dump({
            "framework": "lens_vllm",
            "runtime": "vLLM (TPU backend, Pallas attention)",
            "hardware": "tpu",
