#!/usr/bin/env python3
"""Single-shot continuous-batched vLLM measurement on TPU.
...
TPU-specific knobs are kept identical to measure_vllm.py:
  * `dtype="bfloat16"`, prefix cache + chunked prefill both off (parity
    with measure_vllm.py / inf2 baselines — no in-batch scheduler eviction).
  * `max_num_batched_tokens == max_model_len`. vLLM V1 silently ignores
    `enable_chunked_prefill=False` (see vllm-project/vllm#18547), so the
    real "no chunking" knob is to size the per-step token budget so any
    single prefill fits in one scheduler step. We pass both for clarity.
  * No Neuron/NKI overrides; vLLM's TPU backend manages JAX compilation
    cache internally and uses Pallas attention.

Input  : studies/tpu_v6e_baseline/data/datasets/<dataset>.csv
Output : studies/tpu_v6e_baseline/results/lens_vllm_continuous/<model>/tp<N>/bs<B>/<dataset>.csv

Usage
-----
    python studies/tpu_v6e_baseline/measure_vllm_continuous.py \\
...
def init_llm(model, tp_degree, batch_size, max_model_len, max_num_batched_tokens):
    """vLLM-TPU LLM init. No Neuron-specific overrides — Pallas attention,
    bfloat16, prefix cache + chunked prefill both off for apples-to-apples
    comparison with the inf2 / TPU per-run measurements.
    ...
    max_num_batched_tokens caps the per-step token budget — pinning it to
    max_model_len makes "no chunked prefill" actually hold on V1, where
    ...
    print(f"  dtype=bfloat16  prefix_caching=False  chunked_prefill=False  "
...
def main():
    ...
    p.add_argument("--max-num-batched-tokens", type=int, default=8192,
                   help="per-step token budget. Set ≥ max-model-len to keep "
                        "chunked prefill effectively off on vLLM V1 (which "
                        "silently ignores enable_chunked_prefill=False).")
    p.add_argument("--output-dir", default=None,
                   help="default: results/lens_vllm_continuous/<model>/tp<N>/bs<B>/")
    ...
    llm = init_llm(args.model, args.tp_degree, args.batch_size,
                   args.max_model_len, args.max_num_batched_tokens)
    ...
    with out_json.open("w") as f:
        json.dump({
            ...
            "runtime": "vLLM (TPU backend, Pallas attention, single-shot continuous batching)",
            "hardware": "tpu",
